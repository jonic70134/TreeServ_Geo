...
import { z } from 'zod';
import { GoogleAccessTokenProvider, loadServiceAccount } from './google-auth.mjs';
import { DEFAULT_SPREADSHEET_ID, GoogleSheetsDevlog } from './sheets-client.mjs';

let tokenProviderInstance;
const tokenProvider = {
  async getToken() {
    if (!tokenProviderInstance) {
      tokenProviderInstance = new GoogleAccessTokenProvider({ credentials: await loadServiceAccount() });
    }
...
const server = new McpServer(
  ...
  {
    instructions:
      '將 TreeServ Geo 的開發進度寫入專用 Google Sheet。第一次使用先呼叫 setup_development_log。完成實質開發工作後，依內容呼叫 record_update、record_system_item 或 record_bug_fix；同一變更不要重複記錄。不得修改或刪除非本 MCP 建立的工作表。',
  },
);

const commonAnnotations = { readOnlyHint: false, destructiveHint: false, openWorldHint: false };
...
server.registerTool(
  ...
  {
    title: '建立開發紀錄工作表',
    description: '在指定的 TreeServ Geo Google 試算表中建立並格式化三張專用紀錄工作表；既有其他工作表保持不變。',
    inputSchema: {},
...
server.registerTool(
  ...
  {
    title: '記錄功能與系統項目',
    description: '記錄功能、系統結構或技術棧的新增、變更、決策與淘汰。',
    inputSchema: {
      type: z.enum(['功能', '系統結構', '技術棧']),
      name: requiredText,
      description: requiredText,
      status: z.enum(['規劃中', '開發中', '已上線', '已淘汰']),
      occurred_at: z.string().datetime().optional(),
      module: optionalText,
      technology: optionalText,
      rationale: optionalText,
      impact_or_dependencies: optionalText,
      documentation_link: optionalText,
      git_commit: optionalText,
    },
    annotations: commonAnnotations,
  },
  async (input) => {
    const id = recordId('SYS');
    const result = await devlog.append('system', [
      id,
      timestamp(input.occurred_at),
      input.type,
      input.name,
      ...
      input.technology,
      input.rationale,
      input.impact_or_dependencies,
      input.documentation_link,
      input.git_commit,
      new Date().toISOString(),
    ]);
    return success(`已記錄${input.type}項目：${input.name}`, { id, ...result });
  },
);
...
server.registerTool(
  'record_bug_fix',
  {
    ...
    inputSchema: {
      ...
      fix: requiredText,
      severity: z.enum(['阻斷', '嚴重', '一般', '輕微']),
      status: z.enum(['待處理', '修復中', '已修復', '已驗證', '不修復']),
      discovered_at: z.string().datetime().optional(),
      fixed_at: z.string().datetime().optional(),
      affected_version: optionalText,
      fixed_version: optionalText,
...
server.registerTool(
  'list_recent_development_records',
  {
    ...
    inputSchema: {
      kind: z.enum(['updates', 'system', 'bugs']),
