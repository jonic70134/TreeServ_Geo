...
import {
  auditData, collection, db, doc, normalizeEmail, onSnapshot, serverTimestamp,
  writeBatch, type AccessRole, type User,
} from './firebase';

type Invite = { id: string; email: string; role: 'admin' | 'user'; status: 'pending' | 'accepted' | 'revoked'; invitedByEmail?: string; invitedAt?: any };
type Member = { id: string; uid: string; email: string; displayName: string; role: 'admin' | 'user'; status: 'active' | 'disabled'; acceptedAt?: any };

const roleLabel = (role: string) => role === 'admin' ? '系統管理者' : '一般使用者';
const dateLabel = (value: any) => value?.toDate ? value.toDate().toLocaleString('zh-TW') : '同步中';

export default function AccessManagement({ account, role, notify }: { account: User; role: AccessRole; notify: (message: string) => void }) {
  ...
  useEffect(() => {
    if (!db) return;
    const stopInvites = onSnapshot(collection(db, 'accessInvites'), (snapshot) => setInvites(snapshot.docs.map((item) => ({ id: item.id, ...item.data() } as Invite))), () => setError('無法載入邀請名單。'));
    const stopMembers = onSnapshot(collection(db, 'members'), (snapshot) => setMembers(snapshot.docs.map((item) => ({ id: item.id, ...item.data() } as Member))), () => setError('無法載入使用者名單。'));
    return () => { stopInvites(); stopMembers(); };
  }, []);

  const sortedInvites = useMemo(() => [...invites].sort((a, b) => a.email.localeCompare(b.email)), [invites]);
  const sortedMembers = useMemo(() => [...members].sort((a, b) => a.email.localeCompare(b.email)), [members]);
  ...
  async function createInvite() {
    ...
    try {
      ...
      batch.set(doc(db, 'accessInvites', target), {
        email: target, role: inviteRole, status: 'pending', invitedBy: account.uid,
        invitedByEmail: normalizeEmail(account.email ?? ''), invitedAt: serverTimestamp(), updatedAt: serverTimestamp(),
      });
      batch.set(auditRef, auditData(account, 'invite_create', target, `${target}（${roleLabel(inviteRole)}）`));
      await batch.commit();
      setEmail(''); setInviteRole('user'); notify('邀請已建立，請將邀請連結傳給對方。');
    } catch { setError('邀請建立失敗，請確認帳號權限後重試。'); }
  ...
  function revokeInvite(invite: Invite) {
    setConfirm({ title: '撤銷邀請？', body: `撤銷 ${invite.email} 的邀請後，尚未登入的對方將無法進入系統。`, run: async () => {
      if (!db) return;
      const batch = writeBatch(db), auditRef = doc(collection(db, 'activityLogs'));
      batch.update(doc(db, 'accessInvites', invite.id), { status: 'revoked', updatedAt: serverTimestamp() });
      batch.set(auditRef, auditData(account, 'invite_revoke', invite.email, invite.email));
      await batch.commit(); notify('邀請已撤銷。');
    } });
  }
  ...
  function changeMember(member: Member, action: 'role' | 'status') {
    ...
    setConfirm({
      ...
      run: async () => {
        if (!db) return;
        const batch = writeBatch(db), auditRef = doc(collection(db, 'activityLogs'));
        batch.update(doc(db, 'members', member.id), action === 'role' ? { role: nextRole, updatedAt: serverTimestamp() } : { status: nextStatus, updatedAt: serverTimestamp() });
        batch.set(auditRef, auditData(account, action === 'role' ? 'member_role' : nextStatus === 'active' ? 'member_enable' : 'member_disable', member.uid, `${member.email}（${action === 'role' ? roleLabel(nextRole) : nextStatus}）`));
        await batch.commit(); notify('帳號權限已更新。');
      },
    });
  }
  ...
  async function runConfirmed() {
    ...
    setBusy(true); setError('');
    try { await confirm.run(); setConfirm(undefined); }
    catch { setError('權限更新失敗，請重新整理後再試。'); }
    finally { setBusy(false); }
  }
    ...
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1180, mx: 'auto' }}>
      ...
      <Paper variant="outlined" sx={{ p: 2.5, mb: 3 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ alignItems: { md: 'center' } }}>
          <TextField fullWidth label="受邀 Google 帳號" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" />
          <FormControl sx={{ minWidth: 180 }}><InputLabel>角色</InputLabel><Select label="角色" value={inviteRole} onChange={(event) => setInviteRole(event.target.value as 'admin' | 'user')}><MenuItem value="user">一般使用者</MenuItem>{canInviteAdmin && <MenuItem value="admin">系統管理者</MenuItem>}</Select></FormControl>
          <Button sx={{ minWidth: 150, height: 56 }} variant="contained" startIcon={<PersonAddAlt1Rounded />} disabled={busy} onClick={createInvite}>建立邀請</Button>
        </Stack>
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1.5 }}>系統會建立專屬邀請資格；請複製連結傳給對方。登入電子郵件必須與受邀帳號完全相同。</Typography>
      </Paper>

      <Paper variant="outlined">
        <Tabs value={tab} onChange={(_event, value) => setTab(value)}><Tab label={`邀請名單 ${invites.length}`} /><Tab label={`已加入帳號 ${members.length}`} /></Tabs>
      ...
      <Dialog open={Boolean(confirm)} onClose={() => !busy && setConfirm(undefined)}><DialogTitle>{confirm?.title}</DialogTitle><DialogContent><DialogContentText>{confirm?.body}</DialogContentText></DialogContent><DialogActions><Button disabled={busy} onClick={() => setConfirm(undefined)}>取消</Button><Button disabled={busy} color="error" variant="contained" onClick={runConfirmed}>確認</Button></DialogActions></Dialog>
    </Box>
  );
}
