...
const activeId = ref('demo-urban-banyan');
...
function openCreate(forActive=false){
  ...
  if(forActive && activeLocation.value?.isDemo){ notify('展示案例不會寫入資料庫，請從上方建立真實案場'); return; }
  resetForm(); showCreate.value=true;
}
function openEdit(record:WorkRecord){
  if(activeLocation.value?.isDemo){ notify('展示案例為唯讀資料'); return; }
  editing.value=record; Object.assign(form,{title:record.title,notes:record.notes,imageUrls:record.imageUrls.join('\n'),youtubeUrls:record.youtubeUrls.join('\n'),fileUrls:record.fileUrls.join('\n')}); showCreate.value=true;
}
  ...
  <main class="app-shell">
    ...
    <section class="workspace">
      ...
      <aside v-if="activeLocation" class="record-panel">
        <div class="record-head"><small>目前位置{{activeLocation.isDemo?' · 專業展示案例':''}}</small><span>{{activeLocation.status}}</span></div><h1>{{activeLocation.name}}</h1><p class="address">{{activeLocation.address}}</p><div class="record-summary"><span>{{activeRecords.length}} 筆紀錄</span><button v-if="role!=='guest'" @click="openCreate(true)">＋ 新增</button></div>
