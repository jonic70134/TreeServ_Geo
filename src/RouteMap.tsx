...
export default function RouteMap({
  ...
  function draw() {
    ...
    if (points.length > 1)
      L.polyline(points, { color: '#e25a32', weight: 5 }).addTo(layer.current);
    points.forEach((point, index) => {
      const label =
        index === 0 ? 'A' : index === points.length - 1 ? 'B' : String(index);
      ...
      for (let i = 0; i < count; i += 1) {
        const mid = map.current!.layerPointToLatLng(
          a.add(b.subtract(a).multiplyBy((i + 0.5) / count)),
        );
        L.marker(mid, {
          interactive: false,
          icon: L.divIcon({
  ...
  useEffect(() => {
    draw();
    if (value.length > 1 && map.current)
      map.current.fitBounds(L.latLngBounds(value), {
        padding: [32, 32],
        maxZoom: 17,
      });
    ...
    <Stack spacing={1.5}>
      {editable && (
        <Typography variant="body2">
          依序點選 A 出發點、轉彎途經點，最後一點為 B 終點；標記可拖曳調整。
        </Typography>
      )}
      ...
      {value.length > 0 && (
        <Typography variant="body2" sx={{ fontWeight: 700 }}>
          A 出發點 → {Math.max(0, value.length - 2)} 個途經點 → B 終點
        </Typography>
      )}
      <Typography variant="caption" color="text.secondary">
        手動進場指引不會自動貼齊道路，請確認實際路寬、限高與通行條件。
      </Typography>
    </Stack>
  );
}
