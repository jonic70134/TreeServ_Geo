  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      <AppBar position="sticky" color="inherit" elevation={0} sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Toolbar sx={{ gap: 1.5 }}>
          <Stack direction="row" spacing={1.2} sx={{ alignItems: 'center', minWidth: { md: 210 }, cursor: 'pointer' }} onClick={() => setView('map')}><Avatar variant="rounded" sx={{ bgcolor: 'primary.main' }}><MapRounded /></Avatar><Box sx={{ display: { xs: 'none', sm: 'block' } }}><Typography sx={{ fontWeight: 850, lineHeight: 1 }}>TreeServ Geo</Typography><Typography variant="caption" color="text.secondary">案場工作紀錄</Typography></Box></Stack>
          <TextField size="small" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜尋地點、地址或紀錄…" sx={{ flex: 1, maxWidth: 620 }} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchRounded /></InputAdornment> } }} />
          <Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 1.5 }}>
            <Button sx={{ display: { xs: 'none', md: 'inline-flex' } }} variant="contained" startIcon={<AddRounded />} onClick={() => openCreate(false)}>建立工作紀錄</Button>
            {canManage && <Button sx={{ display: { xs: 'none', lg: 'inline-flex' } }} color="secondary" variant="contained" startIcon={<DescriptionRounded />} onClick={() => setView('plan')}>製作計畫書</Button>}
            <Tooltip title="開啟選單"><IconButton onClick={(event) => setMenuAnchor(event.currentTarget)} aria-label="開啟功能選單"><MenuRounded /></IconButton></Tooltip>
          </Box>
        </Toolbar>
      </AppBar>
